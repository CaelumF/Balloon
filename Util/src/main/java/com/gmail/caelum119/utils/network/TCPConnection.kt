 ...
 * TODO: Update doc comments from separation of UDPConnection
 */
class TCPConnection(val TCPSocket: Socket) : Connection() {
    ...
    private val outTCPStream: DataOutputStream
     ...
     */
    companion object {
        val postInit: ArrayList<(TCPConnection) -> Unit> = ArrayList()
    }

    init {
        outTCPStream = DataOutputStream(TCPSocket.outputStream)
    ...
    override fun sendPacketImmediately(vararg data: Byte) {
        outTCPStream.writeInt(data.size)
        outTCPStream.write(data)
    }
    ...
    override fun waitForPacket(): ByteArray {
        ...
        val inputStream = DataInputStream(TCPSocket.inputStream)
        val length: Int = inputStream.readInt()
        receiveBytes = ByteArray(length)
        if (length > 0) {
            inputStream.readFully(receiveBytes)
        }
